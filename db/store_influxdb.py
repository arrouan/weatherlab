...
for nd in range(0,si_d):
  ...
  new_recs[nd]['measurement'] = "boulder" 
  fld='TIME'
  mdate=data[nd][NOAA_fields[fld][0]:NOAA_fields[fld][1]].strip()
  new_recs[nd]['time'] = datetime.datetime.strptime(str(mdate), '%Y%m%d%H%M')
  #  new_recs[nd]['time'] = mdate
  new_recs[nd]['fields'] = {} 
  for fld in storefields:
    val = data[nd][NOAA_fields[fld][0]:NOAA_fields[fld][1]].strip()
    if not re.search('\*', val):
      if fld == "SKC":
        new_recs[nd]['fields'][fld] = val
      else:
        new_recs[nd]['fields'][fld] = num(val)
      

client.write_points(new_recs)
